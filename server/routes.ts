...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/client/stage-invoices/:stageId/download", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      doc.text('Invoice #:', rightColumnX, yPos + 15);
      doc.text('Etapa de Pago:', rightColumnX, yPos + 30);

      doc.fontSize(10).fillColor('#000');
      doc.text(issueDate, rightColumnX + 70, yPos);
      doc.text(invoiceNumber, rightColumnX + 70, yPos + 15);
      doc.text(`${stageNumber} de ${totalStages}`, rightColumnX + 70, yPos + 30);
  ...
  app.get("/api/client/stage-invoices/:stageId/download-resimple", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const boletaNumber = `BSP-${new Date().getFullYear()}-${String(stageNumber).padStart(6, '0')}`;
      const issueDate = new Date().toLocaleDateString('es-PY');
      ...
      // Set response headers
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="SoftwarePar_Boleta_RESIMPLE_${boletaNumber}.pdf"`);

      // Handle PDF stream errors
      doc.on('error', (error) => {
        console.error('Error generating RESIMPLE PDF:', error);
        if (!res.headersSent) {
          res.status(500).json({ message: "Error generando PDF RESIMPLE" });
        }
        return;
      });
      ...
      doc.rect(leftMargin, 40, contentWidth, 80).strokeColor('#2563eb').lineWidth(2).stroke();
      
      // Company name and title in header box
      doc.fontSize(20).fillColor('#2563eb').text('SoftwarePar S.R.L.', leftMargin + 20, 60);
      doc.fontSize(16).fillColor('#dc2626').text('BOLETA RESIMPLE', pageWidth - 200, 60);
      doc.fontSize(12).fillColor('#374151').text('Desarrollo de Software Profesional', leftMargin + 20, 85);
      doc.fontSize(10).fillColor('#6b7280').text('Paraguay • +595 985 990 046', leftMargin + 20, 100);

      // Boleta number prominently displayed
      let yPos = 140;
      doc.rect(leftMargin, yPos, contentWidth, 30).fillColor('#f3f4f6').fill().strokeColor('#2563eb').stroke();
      doc.fontSize(14).fillColor('#1f2937').text(`BOLETA RESIMPLE N° ${boletaNumber}`, leftMargin, yPos + 8, { align: 'center', width: contentWidth });

      // Required data section with boxes
      yPos = 190;
      doc.fontSize(14).fillColor('#1f2937').text('DATOS OBLIGATORIOS SEGÚN SET', leftMargin, yPos);
      ...
      // 1. Fecha
      doc.rect(leftMargin, yPos, labelWidth, boxHeight).fillColor('#e5e7eb').fill().strokeColor('#9ca3af').stroke();
      doc.rect(leftMargin + labelWidth, yPos, contentWidth - labelWidth, boxHeight).strokeColor('#9ca3af').stroke();
      doc.fontSize(11).fillColor('#374151').text('1. FECHA DE EMISIÓN:', leftMargin + 10, yPos + 7);
      doc.fontSize(11).fillColor('#000').text(issueDate, leftMargin + labelWidth + 10, yPos + 7);

      yPos += boxHeight;
      // 2. RUC/Cédula
      doc.rect(leftMargin, yPos, labelWidth, boxHeight).fillColor('#e5e7eb').fill().strokeColor('#9ca3af').stroke();
      doc.rect(leftMargin + labelWidth, yPos, contentWidth - labelWidth, boxHeight).strokeColor('#9ca3af').stroke();
      doc.fontSize(11).fillColor('#374151').text('2. RUC/CÉDULA DEL CLIENTE:', leftMargin + 10, yPos + 7);
      doc.fontSize(11).fillColor('#000').text(req.user!.email.includes('@') ? '[Cliente debe proporcionar]' : req.user!.email, leftMargin + labelWidth + 10, yPos + 7);

      yPos += boxHeight;
      // 3. Nombre
      doc.rect(leftMargin, yPos, labelWidth, boxHeight).fillColor('#e5e7eb').fill().strokeColor('#9ca3af').stroke();
      doc.rect(leftMargin + labelWidth, yPos, contentWidth - labelWidth, boxHeight).strokeColor('#9ca3af').stroke();
      doc.fontSize(11).fillColor('#374151').text('3. NOMBRE Y APELLIDO:', leftMargin + 10, yPos + 7);
      doc.fontSize(11).fillColor('#000').text(req.user!.fullName, leftMargin + labelWidth + 10, yPos + 7);

      yPos += boxHeight;
      // 4. Monto
      doc.rect(leftMargin, yPos, labelWidth, boxHeight).fillColor('#e5e7eb').fill().strokeColor('#9ca3af').stroke();
      doc.rect(leftMargin + labelWidth, yPos, contentWidth - labelWidth, boxHeight).strokeColor('#9ca3af').stroke();
      doc.fontSize(11).fillColor('#374151').text('4. MONTO TOTAL:', leftMargin + 10, yPos + 7);
      doc.fontSize(12).fillColor('#dc2626').text(`$ ${parseFloat(stage[0].amount).toFixed(2)} USD`, leftMargin + labelWidth + 10, yPos + 7);

      // Service description section
      yPos += 50;
      doc.rect(leftMargin, yPos, contentWidth, 60).fillColor('#f8f9fa').fill().strokeColor('#2563eb').stroke();
      doc.fontSize(12).fillColor('#1f2937').text('DESCRIPCIÓN DEL SERVICIO:', leftMargin + 10, yPos + 10);
      doc.fontSize(11).fillColor('#374151').text(`Desarrollo de Software - ${stage[0].stageName}`, leftMargin + 10, yPos + 28);
      doc.fontSize(10).fillColor('#6b7280').text(`Etapa ${stageNumber} de ${totalStages} del proyecto: ${project.name}`, leftMargin + 10, yPos + 44);

      // Important info section
      yPos += 80;
      doc.fontSize(12).fillColor('#1f2937').text('INFORMACIÓN IMPORTANTE:', leftMargin, yPos);
      yPos += 20;
      
      const infoItems = [
        '✓ Boleta RESIMPLE según normativa SET Paraguay',
        '✓ Exenta de IVA - No aplicable para servicios digitales',
        '✓ Válida como comprobante fiscal de pago',
        `✓ Estado del pago: PAGADO el ${stage[0].paidAt ? new Date(stage[0].paidAt).toLocaleDateString('es-PY') : issueDate}`,
        `✓ Método de pago: ${stage[0].paymentMethod || 'Transferencia Bancaria'}`
      ];
      ...
      // Issuer section
      yPos += 20;
      doc.rect(leftMargin, yPos, contentWidth, 70).strokeColor('#2563eb').stroke();
      doc.fontSize(12).fillColor('#2563eb').text('DATOS DEL EMISOR', leftMargin + 10, yPos + 10);
      
      yPos += 25;
      doc.fontSize(10).fillColor('#000');
      doc.text('SoftwarePar S.R.L.', leftMargin + 10, yPos);
      doc.text('RUC: [En proceso de registro]', leftMargin + 10, yPos + 12);
      doc.text('Actividad: Desarrollo de Software y Aplicaciones', leftMargin + 10, yPos + 24);
      doc.text('Email: softwarepar.lat@gmail.com • Tel: +595 985 990 046', leftMargin + 10, yPos + 36);
      ...
      doc.rect(leftMargin, yPos, contentWidth, 40).fillColor('#e0f2fe').fill().strokeColor('#0ea5e9').stroke();
      doc.fontSize(11).fillColor('#0369a1').text('📄 DOCUMENTO EMITIDO ELECTRÓNICAMENTE', leftMargin, yPos + 8, { align: 'center', width: contentWidth });
      doc.fontSize(10).fillColor('#0369a1').text(`Generado el: ${timestamp}`, leftMargin, yPos + 23, { align: 'center', width: contentWidth });

      // Footer
      yPos += 60;
      doc.fontSize(14).fillColor('#059669').text('¡GRACIAS POR SU PREFERENCIA!', leftMargin, yPos, { align: 'center', width: contentWidth });
      doc.fontSize(11).fillColor('#2563eb').text('SoftwarePar - Transformando ideas en soluciones digitales', leftMargin, yPos + 20, { align: 'center', width: contentWidth });
